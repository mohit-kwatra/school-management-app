<?php
    ...
    if( $_SERVER['REQUEST_METHOD'] == "POST" )
    {
        if( isset($_POST['searchRollNo']) && !empty(trim($_POST['searchRollNo'])) )
        {
            ...
            $query = "UPDATE `fee_status` SET `status` = 'Paid' WHERE `student_roll_no` = '$roll_no';";
...
<head>
    ...
    <title>Update Student's Fee Status</title>
    <!-- Bootstrap CSS -->
    ...
    <div class="container-fluid" style="padding: 0;">
        ...
        <div class="row justify-content-center" style="max-width: 100%">
            <div class="col-8 py-3">
                <h4 class="text-center text-secondary py-3" style="text-decoration: underline;">Update Student's Fee Status</h4>
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        <form method="POST" class="input-group border-top border-bottom py-3">
                            <input type="number" name="searchRollNo" id="searchRollNo" class="form-control" required placeholder="Student's Roll Number" autocomplete="off" required>
                            <button type="submit" name="submit" class="btn btn-primary input-group-text">Update If Paid</button>
                        </form>
                    </div>
                </div>
            </div>
